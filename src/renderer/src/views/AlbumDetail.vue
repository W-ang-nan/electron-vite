    ...
    <div class="albumdetail">
        <div class="header">
            <div class="img">
                ...
                <img v-lazy="title?.blurPicUrl">
            </div>
            <div class="title">
                <h3>{{ title?.name }}</h3>
                <div class="t">{{ title?.artist.name }}</div>
                <div class="t">{{ dateTime(title?.publishTime) }}</div>
                <div class='btn'>
                    <el-button :icon="VideoPlay" round color='#059669'>播放全部</el-button>
        ...
        <div class='list'>
            <el-tabs v-model="activeName" class="demo-tabs" @tab-click="handleClick">
                <el-tab-pane label="歌曲" name="first">
                    ...
                    <div v-for="(item, index) in data" :key='index' class='song' @mouseenter='showhover(index)'
...
import { useAlbumDetail } from '../utils/api';
import { useRoute } from 'vue-router';
import type { album, songs } from '../interfaces/songs';
import { dateTime, time } from '../utils/time';
